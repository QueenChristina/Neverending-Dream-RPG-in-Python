...
#Font and text:
#try this one, but the line list is generated by splitting by text_index. https://stackoverflow.com/questions/32590131/pygame-blitting-text-with-an-escape-character-or-newline
#https://github.com/ColdrickSotK/yamlui/blob/master/yamlui/util.py#L82-L143
#try to split() or append into words so text wrap is not cutting words in half. https://stackoverflow.com/questions/22767509/python-get-the-x-first-words-in-a-string
text_Toggle = False
text_finished = False
text_frame = 0
text_index = 0
...
def message(text):
    ...
    if text_index == len(text):
        ...
        if text_frame >= text_fps:
            currentframe=0
            text_index += 1
    #optimize this mess of 'if ...elses' later. can do a function concerning n lines, but better if you split by words too
...
while True:
    ...
    for thing in inanimate_objects_list:
        #two for loops DON'T work at the same time
        #also, always will be a thing that player is NOT near, will falsely trigger true as not just indicating one object at a time
        #consider iteratable order in list, messes things up
