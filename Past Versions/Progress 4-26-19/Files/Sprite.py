...
class Sprite(pygame.sprite.Sprite):
    ...
    def changeimage(self):
        ...
        # this is necessary so the self.rect.x does not constantly call 0; it CENTERS rect at the image POSITION so that self.rect.x changes
        # Need this for collision to work properly
    ...
    def update(self):
        self.animate()
        self.changeimage()
        # find a way to load self.mask at initialization later, loading every iteration takes TOO LONG
...
class Player(Sprite):
    ...
    def iscollide(self, group):
        global collideList
        # return self.rect.colliderect(sprite.rect) falsely returns true as get_rect() returns image dimensions, not position
        # trying something else, the False paramter indicates false for "remove sprite from group if collides?"
        # it compares 1) sprite with 2) GROUP of sprites to see if it is colliding and 3) what to do if collide
        # it RETURNS a LIST of the sprites from the group that is colliding with the sprite
        # we can say: if collideList: stuff; it will return TRUE is the list contains something
        # use pygame.sprite.collide_rect(player, sprite) if checking collisions between individual objects WITHOUT the .rect, will return bool
        # use http://www.pygame.org/docs/ref/sprite.html#pygame.sprite.spritecollide or https://stackoverflow.com/questions/13995513/object-is-not-iterable
        # to help you. Understand what offset means for pixel perfect collisions with mask. or use pygame.sprite.collide_mask()         
        collideList = pygame.sprite.spritecollide(self, group, False)
    def update(self):
        self.move()
        self.animate()
        self.playerimage()
        # change later; not working well, see https://stackoverflow.com/questions/46862739/sprite-mask-collision-problems-in-pygame; understand OFFSET
        self.mask = pygame.mask.from_surface(self.image)
...
objPerson = Sprite(imgPerson, 1, (500, 500), False)
